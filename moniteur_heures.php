...
<div class="tab">
  ...
  //         </tr>
          // <?php
          echo "<script type='text/javascript'>alert('id session'+" . $_SESSION['id'] . ");</script>";
          $stmt = $conn->prepare('SELECT id_c,debut,fin,id_m,etat FROM h_code where id_m=?');
      ...
      <div class="container">
        ...
        <table border="1">
          ...
          $stmt = $conn->prepare('SELECT id_c,debut,fin,id_m,id_v,etat FROM h_conduite where id_m=?');
          $stmt->bind_param('i', $_SESSION['id']);
          $stmt->execute();
          $result = $stmt->get_result();
          while ($row = $result->fetch_assoc()) {
