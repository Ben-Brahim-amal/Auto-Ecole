...
<body>
  ...
  <main>
    <section id="about" class="about">
      ...
      <div class="container">
        <h1>Les Candidats non inscrits</h1> <!-- moniteur_vue_candidats.php   $row['CIN'] ?> -->
        <?php
        $stmt = $conn->prepare('SELECT nom,prenom,id_c,img_path FROM candidats where formation="non inscrit"');
        $stmt->execute();
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          echo "<a href='admin_vue_candidats.php?id_c=" . $row['id_c'] . "'>  Informations</a></td>";
          echo '<br></figure>';
        }
      ...
      <div class="container">
        <h1>Les Ancients Candidats</h1> <!-- admin_vue_candidats.php   $row['CIN'] ?> -->
        <?php
        $stmt = $conn->prepare('SELECT nom,prenom,id_c,img_path FROM candidats where formation="achevee"');
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result) {
          echo '<ol style="display: flex; flex-wrap: wrap; column-gap: 50px;  width:90%; align-content: flex-start;">';
