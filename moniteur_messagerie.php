    ...
    <div class="container d-flex align-items-center">
      ...
      <nav id="navbar" class="navbar order-last order-lg-0">
        <ul>
          ...
          <li><a class="active" href="moniteur_messagerie.php">Messages</a></li>
  ...
  <main>
    <section id="about" class="about">
      ...
      <div class="container">
        ...
        <?php
        $stmt = $conn->prepare('SELECT nom,prenom,id_c FROM candidats where id_m=?');
        ...
        while ($row = $result->fetch_assoc()) {
          ...
          echo "<a href='moniteur_messagerie.php?id_c=" . $row['id_c'] . "'>  box</a></td>";
          echo '<br>';
          // $id_c = $row['id_c'];
        ...
        if (!empty($_GET)) {
          $id_c = $_GET['id_c'];
          $sender = 'moniteur';
          array_push($_SESSION['sender'], $id_m, $id_c, $sender); ?>
          <div class="container" data-aos="fade-up">
            <h2>Messages</h2>
            <?php
            $stmt = $conn->prepare('SELECT * from messagerie where id_c=? and id_m=? order by timestamp');
            $stmt->bind_param('ii', $id_c, $id_m);
            $stmt->execute();
            $result = $stmt->get_result();
            foreach ($result as $row) {
              if ($row['sender'] == 'moniteur') {
                echo '            ' . '<h5 style="align:right; color:blue;">' . $row['content'] . '</h5>';
              } elseif ($row['sender'] == 'candidat') {
                echo '<h5 style="align:left; font-color:black;">' . $row['content'] . '</h5>';
              }
